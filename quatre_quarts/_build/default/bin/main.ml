...
let max_profit f = match f with (* pattern matching para selecionar apenas o primeiro elemento do tuplo, ou seja, o preço máximo *)
    | (h,_) -> h
(* função principal *)                                                    (* tuplo de int, int array array (matriz de inteiros) *)
let quatre_quarts_price n slices = (* tipagem: int -> (int * int) array -> int * int array array *)
  let num_slices = Array.length slices in 
    let m = Array.make (num_slices + 1) [||] in (* criação do array dos preços máximos *)
    for i = 0 to num_slices do
      m.(i) <- Array.make (n + 1) 0 (* inicializa todos os elementos da matriz com 0 *)
    done;
    ...
    for i = 1 to num_slices do
        for j = 1 to n do
          if 
            nslice slices (i - 1) <= j          (* se o tamanho do bolo atual for maior ou igual ao tamanho da fatia, *) 
            then                                (* o algoritmo compara o preço da fatia com o preço máximo possível *)
            let p = price slices (i - 1) in     (* para um bolo com o tamanho atual menos o tamanho da fatia *)
            m.(i).(j) <- max                    (* e escolhe o máximo entre esses dois valores *)
                (m.(i - 1).(j))            
                (m.(i).(j - nslice slices (i - 1)) + p) 
          else 
            m.(i).(j) <- m.(i - 1).(j)          (* se o tamanho do bolo atual for menor que o tamanho da fatia, *)
        done                                    (* o preço máximo possível permanece o mesmo do bolo anterior. *)
    done;
    (m.(num_slices).(n), m)
